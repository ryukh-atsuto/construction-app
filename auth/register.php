...
// Hardcoded admin authentication key
define('ADMIN_AUTH_KEY', 'admin123');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } elseif ($role === 'admin' && $admin_key !== ADMIN_AUTH_KEY) {
        $error = "Invalid admin authentication key. Please contact system administrator.";
    } else {
        try {
    ...
    <div class="hidden lg:flex lg:w-1/2 relative bg-brand-900 items-center justify-center overflow-hidden">
        <div class="absolute inset-0 z-0">
            <img class="w-full h-full object-cover opacity-30 mix-blend-overlay"
                src="https://images.unsplash.com/photo-1590644365607-1c5a38fc43e0?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80"
        ...
        <div class="relative z-10 p-12 text-white max-w-lg">
            <div class="mb-6">
                <img src="../assets/images/logo.png" alt="Logo"
                    class="h-16 w-auto bg-white/10 p-2 rounded-lg backdrop-blur-sm border border-white/20">
            </div>
        ...
        <!-- Decorative Shapes -->
        <div
            class="absolute -top-24 -right-24 w-64 h-64 bg-accent-500 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob">
        </div>
        <div
            class="absolute bottom-0 left-0 w-72 h-72 bg-brand-500 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob animation-delay-2000">
        </div>
    </div>

    <!-- Right Side: Registration Form -->
    <div
        class="w-full lg:w-1/2 flex items-center justify-center p-8 overflow-y-auto bg-white dark:bg-brand-900 text-gray-900 dark:text-white transition-colors duration-500">
        ...
        <div class="max-w-md w-full space-y-6">
            <div class="text-center lg:text-left">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-brand-200">
                    <?= $t['already_have'] ?> <a href="login.php"
                        class="font-medium text-brand-600 dark:text-brand-400 hover:text-brand-500 dark:hover:text-brand-300 underline"><?= $t['signin_link'] ?></a>
                </p>
            </div>
            ...
            <?php if ($success): ?>
                <div class="bg-green-50 dark:bg-green-900/30 border-l-4 border-green-500 p-4 rounded-r-md">
                    <p class="text-sm text-green-700 dark:text-green-200 font-medium"><?= $success ?> <a href="login.php"
                            class="underline">Login now</a></p>
                </div>
            <?php else: ?>
                ...
                <form class="mt-8 space-y-4" action="" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-brand-100 mb-1"><?= $t['fullname'] ?></label>
                            <input name="name" type="text" required
                                class="appearance-none block w-full px-4 py-3 border border-gray-300 dark:border-brand-700 rounded-lg placeholder-gray-400 dark:placeholder-brand-500/50 bg-white dark:bg-brand-950/50 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition"
                                placeholder="John Doe">
                        </div>

                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-brand-100 mb-1"><?= $t['email'] ?></label>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-brand-100 mb-1"><?= $t['password'] ?></label>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-brand-100 mb-1"><?= $t['phone'] ?></label>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-brand-100 mb-1"><?= $t['role_label'] ?></label>
                            <select name="role" id="role-select" required onchange="toggleAdminKey()"
                                class="block w-full px-4 py-3 border border-gray-300 dark:border-brand-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition bg-white dark:bg-brand-950/50 text-gray-900 dark:text-white">
                        ...
                        <!-- Admin Authentication Key (Hidden by default) -->
                        <div id="admin-key-field" class="hidden">
                            <label class="block text-sm font-medium text-gray-700 dark:text-brand-100 mb-1">
                                <i class="fas fa-key mr-1 text-amber-500"></i> Admin Authentication Key
                            </label>
                            <input name="admin_key" type="password" id="admin-key-input"
                                class="appearance-none block w-full px-4 py-3 border border-amber-400 dark:border-amber-600 rounded-lg placeholder-gray-400 dark:placeholder-brand-500/50 bg-amber-50 dark:bg-amber-900/20 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition"
                                placeholder="Enter admin key to register as administrator">
                            <p class="mt-1 text-xs text-amber-600 dark:text-amber-400">Contact your system administrator for
