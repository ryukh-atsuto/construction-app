...
try {
    ...
    // 2. Update Primary Users Table - Build dynamic query based on available fields
    $user_fields = [];
    $sql_parts = [];

    // Required field
    if (isset($_POST['name'])) {
        $user_fields['name'] = $_POST['name'];
        $sql_parts[] = "name = :name";
    }

    // Optional fields - only add if present in POST
    if (isset($_POST['phone'])) {
        $user_fields['phone'] = $_POST['phone'];
        $sql_parts[] = "phone = :phone";
    }
    if (isset($_POST['age']) && $_POST['age'] !== '') {
        $user_fields['age'] = $_POST['age'];
        $sql_parts[] = "age = :age";
    }
    if (isset($_POST['gender'])) {
        $user_fields['gender'] = $_POST['gender'];
        $sql_parts[] = "gender = :gender";
    }
    if (isset($_POST['national_id'])) {
        $user_fields['national_id'] = $_POST['national_id'];
        $sql_parts[] = "national_id = :national_id";
    }
    if (isset($_POST['address'])) {
        $user_fields['address'] = $_POST['address'];
        $sql_parts[] = "address = :address";
    }

    if ($profile_picture) {
        $sql_parts[] = "profile_picture = :profile_picture";
    ...
    switch ($role) {
        case 'owner':
            ...
            if ($doc_path) {
                $detail_sql .= ", verification_doc_path = :doc";
                $update_fields['doc'] = $doc_path;
            }
            break;
        ...
        case 'manager':
            ...
            if ($doc_path) {
                $detail_sql .= ", leadership_cert_path = :doc";
                $update_fields['doc'] = $doc_path;
            }
            break;
        ...
        case 'worker':
            ...
            if ($doc_path) {
                $detail_sql .= ", safety_cert_path = :doc";
                $update_fields['doc'] = $doc_path;
            }
            break;
