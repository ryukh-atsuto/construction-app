...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <?php if (count($consultations) > 0): ?>
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row justify-between gap-6 hover:shadow-md transition-shadow">
                <div class="flex gap-4">
                    <div class="flex-shrink-0">
                         <div class="h-12 w-12 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-lg">
                            <?= date('d', strtotime($consult['consultation_date'])) ?>
                        </div>
                         <div class="text-center text-xs text-indigo-600 uppercase font-bold mt-1">
                            <?= date('M', strtotime($consult['consultation_date'])) ?>
                        </div>
                    </div>
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h3 class="font-bold text-gray-900 text-lg"><?= htmlspecialchars($consult['project_name']) ?></h3>
                             <?php renderStatusBadge($consult['status']); ?>
                        </div>
                        <p class="text-gray-600"><i class="fas fa-map-pin mr-1 text-gray-400"></i> <?= htmlspecialchars($consult['location']) ?></p>
                    </div>
                </div>
