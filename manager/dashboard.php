        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <?php if (count($projects) > 0): ?>
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-start gap-4">
                            <div class="rounded-lg bg-brand-50 p-3 hidden md:block">
                                <i class="fas fa-building text-brand-600 text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-lg font-bold text-gray-900"><?= htmlspecialchars($project['project_name']) ?></h3>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                             <a href="manager/view_project.php?id=<?= $project['project_id'] ?>" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-4 border-b border-gray-100 bg-gray-50">
                    <h3 class="font-bold text-gray-900">Service Requests</h3>
                </div>
                    ...
                    <?php if (count($services) > 0): ?>
                        ...
                        <div class="p-4 hover:bg-gray-50 transition-colors">
                            <div class="flex justify-between items-start mb-1">
                                <h4 class="text-sm font-bold text-gray-900 line-clamp-1"><?= htmlspecialchars($service['project_name']) ?></h4>
                                <span class="text-xs text-gray-400"><?= date('M d', strtotime($service['request_date'])) ?></span>
                            </div>
                            <p class="text-sm text-gray-500 mb-3 line-clamp-2"><?= htmlspecialchars($service['service_description']) ?></p>
                            <a href="manager/view_service.php?id=<?= $service['service_id'] ?>" class="text-xs font-semibold text-brand-600 hover:text-brand-800">
                                Assign Worker &rarr;
                            </a>
                        </div>
                        <?php endforeach; ?>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                    <h3 class="font-bold text-gray-900">Team Insights</h3>
                    <span class="text-xs font-medium px-2 py-1 bg-brand-100 text-brand-700 rounded-full">Live Stats</span>
                </div>
                
                <div class="p-4 space-y-6">
                    <!-- Available Workers -->
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Available for Hire</h4>
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between group">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-gray-900 group-hover:text-brand-600 transition-colors"><?= htmlspecialchars($worker['name']) ?></p>
                                        <p class="text-xs text-gray-500"><?= htmlspecialchars($worker['skillset']) ?></p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-bold text-gray-900">$<?= $worker['hourly_rate'] ?>/hr</p>
                    ...
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Top Performers</h4>
                        <div class="space-y-3">
                            <?php foreach ($top_performers as $i => $worker): ?>
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    <span class="text-sm font-bold <?= $i < 3 ? 'text-amber-500' : 'text-gray-400' ?>">#<?= $i+1 ?></span>
                                    <div>
                                        <p class="text-sm font-bold text-gray-900"><?= htmlspecialchars($worker['name']) ?></p>
                                        <p class="text-[10px] text-gray-500"><?= htmlspecialchars($worker['skillset']) ?></p>
                                    </div>
                                </div>
                                <div class="px-2 py-1 bg-green-50 text-green-700 rounded text-xs font-bold">
                                    <?= $worker['rating'] ?> ★
                                </div>
                            </div>
                            <?php endforeach; ?>
